...
# Define Room number for each marker
def convertMarkertoRoom(marker_name):

    if marker_name == 'aruco_marker[[1]]':
        room_name = 'room A'
    elif marker_name == 'aruco_marker[[2]]':
        room_name = 'room B'
    elif marker_name == 'aruco_marker[[3]]':
        room_name = 'room C'
    else:
        print("Cannot identify any marker!!!")
    ...
    def marker_detection(self, depth_img):
        ...
        if np.shape(corners)[0] > 0:
                for i in range(np.shape(corners)[0]):
                    ...
                    marker_name = 'aruco_marker' + str(ids)
                    # print(marker_name)

                    room_name = convertMarkertoRoom(marker_name)

                    trans_stamp = TransformStamped(header, room_name, trans)
